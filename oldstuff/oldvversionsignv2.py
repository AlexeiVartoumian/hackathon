...
SPREADSHEET_ID = '18AkvPdk1GfcPr5Zwq2dmcSNjKVdIX-ltCrk5FAR_cds'  
#WORKSHEET_NAME = 'Hackathon teams'  
...
def find_next_empty_cell(worksheet):
    """
    Find the next empty cell following the pattern:
    - Fill down each column (max MAX_ROWS rows)
    - Then move to next column
    
    Returns: (row, col) tuple (1-indexed)
    """
...
def lambda_handler(event, context):
    ...
    {
        "action": "add",
        "character": "X"  # Optional - if not provided, generates random
    }
    
    Or just invoke with no event to add a random character
    """
